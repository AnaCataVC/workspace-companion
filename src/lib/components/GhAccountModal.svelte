    ...
    <div class="w-full max-w-sm bg-neutral-900 border border-neutral-800 rounded-xl p-4 shadow-2xl flex flex-col gap-3">
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-1">
          <button
            ...
            class="text-neutral-400 hover:text-neutral-300 p-1 rounded"
            title="Refresh accounts"
          >
            <RefreshCw size={13} class={$isGhLoading ? 'animate-spin text-indigo-400' : ''} />
          </button>
          <button on:click={close} class="text-neutral-400 hover:text-neutral-300 p-1.5 rounded" aria-label="Close">
            <X size={14} />
          </button>
        </div>
      </div>
      ...
      <div class="space-y-1.5 max-h-56 overflow-y-auto">
        {#if $ghAccounts.length === 0}
          <div class="p-3 text-center text-xs text-neutral-400 bg-neutral-950/60 rounded-lg border border-neutral-800">
          ...
          {#each $ghAccounts as acc (acc.username)}
            ...
            >
              <div class="flex items-center gap-2">
                <div class="w-6 h-6 rounded-full bg-neutral-800 flex items-center justify-center font-mono text-[11px] font-bold text-neutral-300">
                  {acc.username.substring(0, 2).toUpperCase()}
                </div>
                <div class="flex flex-col">
                  <span class="font-mono font-medium">{acc.username}</span>
                  <span class="text-[11px] text-neutral-400">{acc.host}</span>
                </div>
              </div>
              ...
              {:else}
                <span class="text-[11px] text-neutral-400 opacity-0 hover:opacity-100 transition-opacity">
                  Switch
                </span>
              {/if}
            </button>
      ...
      <!-- Footer info -->
      <div class="pt-2 border-t border-neutral-800/70 flex justify-between items-center text-[11px] text-neutral-400">
