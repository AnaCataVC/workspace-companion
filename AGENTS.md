...
3. **Tray Auto-Hide**: The floating window toggles on a single tray click (never on `DoubleClick`, which also delivers two clicks) and on Alt+Space. `CloseRequested` hides instead of quitting. It auto-hides on `WindowEvent::Focused(false)` only when not pinned and not busy; the frontend must keep `set_panel_state` in sync whenever a modal opens or a long-running action starts.
